...
/* Compose draws its own preview now, so it asks for the worn skin directly
 * instead of opening the editor, and saves each change as it is made. */
static bool pending_sync = false;
static bool pending_sync_save = false;
...
void android_skin_poll(tenv* env) {
  ...
  skin_sync = pending_sync;
  skin_sync_save = pending_sync_save;
  ...
  /* Unlike commit, a sync never changes screen: it only persists what the
   * Skin tab just changed and tells Compose what is now worn. */
  if (skin_sync_save) save_user_settings(settings);
  if (skin_sync) android_skin_publish_state(env);
}

JNIEXPORT void JNICALL
Java_com_wyrm_omrajput_WyrmActivity_nativeSkinSync(JNIEnv* env, jclass clazz,
                                                   jboolean save) {
  (void)env;
  (void)clazz;
  ...
  SDL_LockMutex(skin_mutex);
  pending_sync = true;
  if (save == JNI_TRUE) pending_sync_save = true;
  SDL_UnlockMutex(skin_mutex);
}
