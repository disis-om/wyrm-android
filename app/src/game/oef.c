...
void time_step(tenv* env) {
  ...
  if (gdata->data.follow_view)
    gdata->data.play_etm = time_sec - gdata->life_started_sec;
  ...
  if (!gdata->data.lagging) {
    if (gdata->data.wfpr && gdata->data.ctm - gdata->data.last_ping_mtm > ARENA_LAG_MS) {
      gdata->data.lagging = true;
    }
  }

  if (gdata->data.lagging) {
    gdata->data.lag_mult = arena_lag_step(gdata->data.lag_mult, true);
    if (gdata->data.lag_mult < .2) gdata->data.lag_mult = .2;
  } else if (gdata->data.lag_mult < 1) {
    gdata->data.lag_mult = arena_lag_step(gdata->data.lag_mult, false);
...
void oef(tenv* env) {
  ...
  for (int i = snakes_len - 1; i >= 0; i--) {
    ...
    if (!o->dead) {
      /* Slither.txt uses asymmetric per-frame convergence here, including the
         tiny epsilon which guarantees that the value reaches its target.
         `tsp` drives boost presentation; head displacement still continues to use
         authoritative `sp` below. */
      if (o->tsp != o->sp) {
        if (o->tsp < o->sp) {
          o->tsp += (o->sp - o->tsp) * .1f;
          o->tsp += 1e-4f;
    ...
    if (o->dead) {
      o->dead_amt += gdata->data.vfr * 0.02f;
      if (o->dead_amt >= 1) {
      ...
      o->alive_amt += gdata->data.vfr * 0.015f;
  ...
  /* AI Mode owns its local opponents and keeps the player manual. The saved
     online Bot toggle must not steer or mutate that offline session. */
  if (!gdata->ai_mode && usrs->hotkeys[HOTKEY_BOT].active &&
      gdata->data.follow_view)
    sbot_go(env);
}
