...
static ai_bot_context ai_contexts[15];
...
void sbot_reset_ai_contexts(void) {
  memset(ai_contexts, 0, sizeof(ai_contexts));
}

void sbot_forget_ai_snake(int snake_id) {
  for (int i = 0; i < 15; ++i)
    if (ai_contexts[i].used && ai_contexts[i].id == snake_id) {
      ai_contexts[i] = (ai_bot_context){0};
      return;
    }
}
...
bool sbot_decide_for_snake(tenv* env, int snake_id, sbot_decision* decision) {
  ...
  snake* controlled = NULL;
  for (int i = 0; i < tdarray_length(gdata->data.snakes); ++i)
    if (gdata->data.snakes[i].id == snake_id && !gdata->data.snakes[i].dead) {
      controlled = gdata->data.snakes + i;
      break;
    }
  if (!controlled) return false;

  ai_bot_context* context = NULL;
  for (int i = 0; i < 15; ++i) {
    if (ai_contexts[i].used && ai_contexts[i].id == snake_id) {
      context = ai_contexts + i;
      break;
    }
    if (!context && !ai_contexts[i].used) context = ai_contexts + i;
  }
  if (!context) return false;
  if (!context->used) {
    context->used = true;
    context->id = snake_id;
    context->state = (bot_state){0};
    context->state.id = snake_id;
    context->state.delay_frame = -1;
    context->state.circle_dir = 1;
    context->state.enable_encircle = true;
    context->state.enable_follow_circle = true;
    context->state.goal = (v2){controlled->xx + cosf(controlled->ang) * 500.0f,
                               controlled->yy + sinf(controlled->ang) * 500.0f};
  }

  bot_state saved = B;
  sbot saved_output = gdata->bot;
  float saved_view_x = gdata->data.view_xx;
  float saved_view_y = gdata->data.view_yy;
  B = context->state;
  B.id = snake_id;
  B.radius_mult = env->usr->usrs.bot_radius_mult;
  B.follow_circle_length = env->usr->usrs.bot_follow_circle_score;
  gdata->data.view_xx = controlled->xx;
  gdata->data.view_yy = controlled->yy;
  gdata->bot.output.accel = false;
  ...
  decision->heading =
      atan2f(B.goal.y - controlled->yy, B.goal.x - controlled->xx);
  decision->accel = gdata->bot.output.accel;
  context->state = B;
  gdata->data.view_xx = saved_view_x;
  gdata->data.view_yy = saved_view_y;
  gdata->bot = saved_output;
  B = saved;
  return isfinite(decision->heading);
}
